...
export default function RankingTable({ data, title, levelLabel, rows, onTerritorioClick, selectedTerritorio }) {
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rowsToShow.map((row, index) => (
                  <tr
                    key={row.territorio}
                    onClick={() => onTerritorioClick?.(row.territorio)}
                    className={`border-b border-neutral-100 transition-all ${
                      onTerritorioClick ? 'cursor-pointer hover:bg-primary-50' : 'hover:bg-primary-50/40'
                    } ${
                      selectedTerritorio === row.territorio
